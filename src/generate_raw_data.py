...
def copy_data_from_storage():
    for dirname, _, filenames in os.walk(os.path.join(LAYER_2_PROCESSED_DATA_PATH, "ltc_1")):
        for filename in filenames:
            if (filename.startswith("no_")):
                fn = filename[3:-4]
            elif (filename.startswith("yes_")):
                fn = filename[4:-4]
            for d, _, fs in os.walk(os.path.join("..", "data", "storage", "hospital data", "yes for test")):
                for f in fs:
                    if f.startswith(fn):
                        copy(os.path.join(d, f), os.path.join(LAYER_2_TRAIN_RAWDATA_PATH, "yes"))
            print(fn) 

generate_raw_video(False)
# generate_video_screenshoot()
# copy_data_from_storage()